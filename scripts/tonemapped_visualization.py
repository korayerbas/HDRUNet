...
for filename in sorted(os.listdir(res_dir)):
    ...
    hdr_image = io.imread_uint16_png(osp.join(res_dir, "{:04d}_gt.png".format(image_id)), osp.join(res_alignratio_dir, "{:04d}_alignratio.npy".format(image_id)))
    ...
    cv2.imwrite(osp.join(res_output_dir, "{:04d}_tone_mapped_gt.png".format(image_id)),  cv2.cvtColor(hdr_image, cv2.COLOR_RGB2BGR))
